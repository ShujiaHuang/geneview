...
def _iter_point(c, iter_num=100, escape_radius=2):                                        
    z = c
    for i in xrange(1, iter_num): # 最多迭代100次                                        
        if abs(z) > escape_radius: break # 半径大于该值则认为逃逸                        
        z = z * z + c                                                                    
...
def _smooth_iter_point(c, iter_num=20, escape_radius=10):                                        
    ...
    mu = i - log(log(abs(z), 2), 2) if abs(z) > 2.0 else i
    return mu # 迭代次数
...
def _mandelbrot(cx, cy, d, n=200, iter_point_func=_iter_point, 
    ...
    m = np.frompyfunc(iter_point_func,
                      3, 1)(c, iter_num, escape_radius).astype(np.float)
    return m
